...
export default async function AnimalPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      <div className="min-h-screen bg-[#FEFAE0] flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-xl p-8 md:p-12 text-center max-w-lg w-full border border-[#1B4332]/5">
          ...
          <p className="text-[#2D6A4F]/70 mb-6">
            El código <span className="font-mono font-bold text-[#1B4332]">{id}</span> no está registrado.
          </p>
          <div className="flex flex-col gap-3">
            <Link href={`/animals/new?qr=${encodeURIComponent(id)}`} className="w-full py-3.5 bg-[#1B4332] text-white rounded-xl font-bold hover:bg-[#2D6A4F] transition-all shadow-lg flex items-center justify-center gap-2">
              <Baby size={20} /> Registrar Nuevo Animal
            </Link>
            <Link href="/" className="w-full py-3.5 bg-white text-[#1B4332] border-2 border-[#1B4332]/20 rounded-xl font-bold hover:bg-[#FEFAE0] transition-all flex items-center justify-center gap-2">
  ...
  const milkRecordsCount = (animal.milk_records as any[])?.length || 0

  const animalImage = animal.image_url || getDefaultImageBySpecies(animal.species)
    ...
    <div className="min-h-screen bg-[#FEFAE0]">
      ...
      <nav className="bg-white/80 backdrop-blur-sm border-b border-[#1B4332]/10 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <Link href="/animals" className="flex items-center gap-2 text-[#1B4332] font-bold hover:text-[#2D6A4F] transition-colors">
            <ArrowLeft size={20} /> Inventario
          </Link>
          <div className="flex gap-3">
            <Link href={`/animals/${animal.id}/edit`} className="flex items-center gap-2 px-4 py-2 bg-[#FEFAE0] text-[#1B4332] border border-[#1B4332]/20 rounded-lg font-bold hover:bg-[#2D6A4F] hover:text-white transition-all">
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-[#1B4332]/10 overflow-hidden">
          <div className="bg-gradient-to-r from-[#1B4332] to-[#2D6A4F] p-8 text-white relative">
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-6">
              ...
              <div className="flex-1">
                ...
                <h1 className="text-4xl md:text-5xl font-extrabold mb-2">{animal.name || 'Sin Nombre'}</h1>
          ...
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-[#FEFAE0] p-5 rounded-2xl border border-[#1B4332]/5">
              ...
              <p className="font-bold text-[#1B4332]">Establo Principal</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          {/* ✅ NUEVO: Estadística de Leche */}
          <div className="bg-white p-6 rounded-2xl border border-[#1B4332]/10 shadow-sm flex items-center justify-between group hover:shadow-md transition-all">
            <div>
              <p className="text-[#BC6C25] font-bold text-sm uppercase tracking-wide mb-1">Producción</p>
              <p className="text-3xl font-extrabold text-[#1B4332]">{milkRecordsCount}</p>
              <p className="text-xs text-[#2D6A4F]/60 mt-1">Pesajes</p>
            </div>
            <div className="w-14 h-14 bg-[#FEFAE0] rounded-xl flex items-center justify-center text-[#BC6C25] group-hover:bg-[#BC6C25] group-hover:text-white transition-colors">
              <Droplet size={28} />
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <section className="bg-white rounded-2xl border border-[#1B4332]/10 shadow-sm p-6">
            <h3 className="text-xl font-bold text-[#1B4332] mb-4 flex items-center gap-2">
              <Pill size={20} className="text-[#BC6C25]" /> Historial Médico
            </h3>
            {medicationsCount > 0 ? (
          ...
          <section className="bg-white rounded-2xl border border-[#1B4332]/10 shadow-sm p-6">
            <h3 className="text-xl font-bold text-[#1B4332] mb-4 flex items-center gap-2">
              <Scale size={20} className="text-[#1B4332]" /> Registro de Pesos
            </h3>
          ...
          <section className="bg-white rounded-2xl border border-[#1B4332]/10 shadow-sm p-6 lg:col-span-2">
            <h3 className="text-xl font-bold text-[#1B4332] mb-4 flex items-center gap-2">
              <Droplet size={20} className="text-[#BC6C25]" /> Registro de Producción Lechera
            </h3>
            {milkRecordsCount > 0 ? (
              <div className="space-y-3">
                {(animal.milk_records as any[]).map((milk: any, i: number) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-[#FEFAE0] rounded-xl border border-[#1B4332]/5">
                    <span className="font-medium text-[#1B4332] text-sm">{milk.notes || 'Producción registrada'}</span>
                    <span className="text-xs font-bold text-[#2D6A4F]/60 bg-white px-2 py-1 rounded">{formatDate(milk.date)}</span>
                  </div>
                ))}
              </div>
            ) : <p className="text-sm text-[#2D6A4F]/50 italic py-4 text-center">No hay registros de producción lechera.</p>}
          </section>
        </div>
